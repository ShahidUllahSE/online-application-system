...
app.get('/api/chairman-applications', async (req, res) => {
  try {
    ...
    const applications = await ChairmanApplication.find({}, 'fullName applicationType registrationNumber sendTo submittedAt message remark semester status paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
...
app.get('/api/teacher-applications', async (req, res) => {
  try {
    const applications = await TeacherApplication.find({}, 'fullName applicationType registrationNumber sendTo submittedAt message status remark semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/batch-advisor-applications', async (req, res) => {
  try {
    const applications = await BatchAdvisorApplication.find({}, 'fullName applicationType sendTo registrationNumber submittedAt message remark status semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory' );
    res.json(applications);
  } catch (error) {
...
app.get('/api/semester-coordinator-applications', async (req, res) => {
  try {
    const applications = await SemesterCoordinatorApplication.find({}, 'fullName applicationType sendTo registrationNumber submittedAt message remark status semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/other-applications', async (req, res) => {
  try {
    const applications = await OtherApplication.find({}, 'fullName applicationType registrationNumber sendTo submittedAt message remark status semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/fyp-supervisor-applications', async (req, res) => {
  try {
    const applications = await FYPSupervisorApplication.find({}, 'fullName applicationType registrationNumber sendTo submittedAt message remark status semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/associate-chairman-applications', async (req, res) => {
  try {
    const applications = await AssociateChairmanApplication.find({}, 'fullName applicationType registrationNumber sendTo submittedAt status message remark semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/convener-disciplinary-committee-applications', async (req, res) => {
  try {
    const applications = await ConvenerDisciplinaryCommitteeApplication.find({}, 'fullName applicationType sendTo registrationNumber status submittedAt message remark semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/convener-scholarship-committee-applications', async (req, res) => {
  try {
    const applications = await ConvenerScholarshipCommitteeApplication.find({}, 'fullName applicationType sendTo registrationNumber status submittedAt message remark semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/coordinator-applications', async (req, res) => {
  try {
    const applications = await CoordinatorApplication.find({}, 'fullName applicationType registrationNumber sendTo submittedAt status message remark semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/mid-exam-rearrangement-committee-applications', async (req, res) => {
  try {
    const applications = await MidExamRearrangementCommitteeApplication.find({}, 'fullName applicationType sendTo registrationNumber status submittedAt message remark semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/all-faculty-members-applications', async (req, res) => {
  try {
    const applications = await AllFacultyMembersApplication.find({}, 'fullName applicationType registrationNumber sendTo status submittedAt message remark semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/cms-operator-applications', async (req, res) => {
  try {
    const applications = await CMSOperatorApplication.find({}, 'fullName applicationType registrationNumber sendTo submittedAt status message remark semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.get('/api/office-assistant-applications', async (req, res) => {
  try {
    const applications = await OfficeAssistantApplication.find({}, 'fullName applicationType registrationNumber status sendTo submittedAt message remark semester paperNumber fypChangeReason paperName forwardTo studentRemark remarkHistory');
    res.json(applications);
  } catch (error) {
...
app.post('/api/applications', async (req, res) => {
  const { fullName, registrationNumber, applicationType, sendTo, message, username, semester,paperNumber,fypChangeReason,paperName,forwardTo,remark,status,studentRemark,remarkHistory } = req.body;
  ...
  try {
    ...
    await ApplicationModel.create({ fullName, registrationNumber, applicationType, sendTo, message, username, semester,paperNumber,fypChangeReason,paperName,forwardTo,remark,status,studentRemark,remarkHistory });
...
app.put('/accept-application/:id', async (req, res) => {
  ...
  const { studentRemark } = req.body;  // Get studentRemark from the request body
  ...
  try {
    ...
    // Move application to completed status and update studentRemark if provided
    application.status = 'completed';
    if (studentRemark) {
      application.studentRemark = studentRemark;
    }
...
app.get('/api/pending-applications', async (req, res) => {
  try {
    const statusCriteria = { status: { $in: ['pending', 'forwarded'] } };
    
    const pendingApplications = await Promise.all([
      ChairmanApplication.find(statusCriteria),
      TeacherApplication.find(statusCriteria),
      BatchAdvisorApplication.find(statusCriteria),
      SemesterCoordinatorApplication.find(statusCriteria),
      OtherApplication.find(statusCriteria),
      FYPSupervisorApplication.find(statusCriteria),
      AssociateChairmanApplication.find(statusCriteria),
      ConvenerDisciplinaryCommitteeApplication.find(statusCriteria),
      ConvenerScholarshipCommitteeApplication.find(statusCriteria),
      CoordinatorApplication.find(statusCriteria),
      MidExamRearrangementCommitteeApplication.find(statusCriteria),
      AllFacultyMembersApplication.find(statusCriteria),
      CMSOperatorApplication.find(statusCriteria),
      OfficeAssistantApplication.find(statusCriteria),
      // Add more as needed
    ]);
    ...
    const transformedApplications = pendingApplications.flat().map(application => ({
      ...
      remarkHistory: application.remarkHistory || [],
      forwardTo: application.forwardTo,
...
app.put('/api/update-application/:id', async (req, res) => {
  const { id } = req.params;
  const { forwardTo, semester, paperNumber, fypChangeReason, paperName, remark, studentRemark } = req.body;
  const userRole = req.headers.userrole;
  ...
  try {
    ...
    if (!roleToModelMap.hasOwnProperty(userRole) || !roleToModelMap.hasOwnProperty(forwardTo.toLowerCase().replace(/ /g, '_'))) {
      return res.status(404).json({ message: 'Application type not supported' });
    }
    ...
    application.remarkHistory = application.remarkHistory || [];
    application.remarkHistory.push({
      role: userRole,
      remark,
      forwardTo,
      timestamp: new Date()
    ...
    // Update current application fields
    application.status = 'forwarded';
    application.remark = '';
    application.studentRemark = studentRemark;
    application.semester = semester;
    application.paperNumber = paperNumber;
    application.fypChangeReason = fypChangeReason;
    application.paperName = paperName;
    application.forwardTo = forwardTo;

  
    await application.save();
  
    // Save a copy of the application in the target model
    const forwardedApplication = new targetApplicationModel({
    ...
    res.json({ message: 'Application forwarded successfully', application });
  } catch (error) {
